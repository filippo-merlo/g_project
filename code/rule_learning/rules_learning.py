...
def my_train_clip_encoder(dt, memory, rule_name, rule):
	# get model
	clip_model, clip_preprocess = clip.load("ViT-B/32", device=device)
	model = CLIP_AE_Encode(hidden_dim_clip, latent_dim, isAE=False)
	if rule_name in memory.keys():
		print("______________ loading_____________________")
		model.load_state_dict(memory[rule_name]['model'])
	...
	while loss > 0.008:
		...
		for i in range(200):
			...
			names_sim, images_sim, names_dif, images_dif = dt.get_batches_for_rules(rule)
	...
	############ save model #########
	with torch.no_grad():
		memory[rule_name] = {'model': model.to('cpu').state_dict(),
						'arch': ['Filter', ['para_block1']],
						'centroid': centroid_sim.to('cpu')
						}
	return memory

def my_eval(dt, memory, rule_name, rule):
	tot = 0
	tot_n = 0
...
def my_clip_train(in_path, out_path, n_split, model_name, source, in_base,
	...
	for i in range(epochs):
		for rule_name,rule in rules.items():  # attr
			print("#################### Learning: " + str(i) + " ----- " + str(rule))
			t_start = time.time()
			memory = my_train_clip_encoder(dt, memory, rule_name, rule)
			t_end = time.time()
			t_dur = t_end - t_start
			t_tot += t_dur
...
if __name__ == "__main__":
	...
	argparser.add_argument('--model_name', '-n', default='rules_mem',
